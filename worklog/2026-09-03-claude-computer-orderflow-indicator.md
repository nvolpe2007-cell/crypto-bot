...
**paper-only** behind a corpse: 228 trades, 0.9% win rate, −$20.14 net, 73.6% fee
drag. Memory `market_structure_signals_verdict` is blunter — intraday order
...
non-loosened gates on fresh data"* — and that is the gap worth filling.
...
one of them is **stateful and live-feed-shaped** — takes an exchange handle or
...
that fixable without new infrastructure — given data, it scores it. A recorder is
...
- `ofi_from_books` — canonical **Cont, Kukanov & Stoikov (2014)** OFI over
...
- `tick_rule_side` / `signed_volume_from_ticks` — Lee-Ready tick-rule side
...
- `bar_signed_volume` — close-location-value **proxy** from OHLCV, explicitly
...
poisons downstream sums: doji bars score 0 not ∞, an empty book scores 0 not NaN,
a zero-variance z-score window scores 0 not ±inf, leading trades with no
...
48 new tests, all passing. Full suite **3620 passed, 9 failed** — the same 9
...
backwards, and a sign error there is invisible downstream — it just makes the
...
in-flight forward test — it is inert until something calls it. Same declaration
